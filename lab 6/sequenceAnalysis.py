...
These dictionaries stores the counts of amino acids, nucleotides.'''
    def __init__ (self, inString=''):#this method initializes 3 dictionaries
        ...
        '''The addSequence method takes DNA or RNA sequences from  string inSeq and 
        + to the count of nucleotides, codons, and amino acids to the corresponding dictionaries.'''
    def addSequence (self, inSeq):
        inSeq = inSeq.upper() #makes sequence upper
        for nuc in inSeq: #iterates over each nucleotide in the input sequence and updates the count in the nucComp dictionary
            if nuc in NucParams.validnucleotides.keys():#+ to count and checking if nucleotides are valid nucleotides
                self.nucComp[nuc] += 1 # +1 to the count
        RNAsequence = inSeq.replace('T','U') #convert to RNA sequence by replacing thymine with uracil 
...
# ProteinParam.aa2mw['A'] or ProteinParam.mwH2O
    ...
    def __init__ (self, proteinsequence):   #T.A helped me construct innit.
        ...
        self.ProteinInput = ''.join(AminoAcidlist).upper() #return string as upper charc.
        self.aaComp = {}
        for aa in self.aa2mw.keys():  #Finds + stores the values
    ...
    def aaCount(self):
      #init a counter for the number of amino acids
      #Forloop over cada character in the ProteinInput string
      #Return final count amino acids
      return sum(1 for aa in self.ProteinInput if aa.upper() in self.aa2mw)
      
    ''' The pI function iterate over range of pH values + calc. the abs charge of the
      molecule at each pH using the charge method
      The pH with the lowest absolute charge is returned as the pI.
      function stops iterating when the difference
      between the current pH and 14.01 is greater than 14.01.'''
    def pI(self):
      #Set the init value of pH to 0 and the initial max charge to infinity using the infinity float function 
        pH = 0
        MaxCharge = float("Infinity")
      #Loop over range of pH values from 0.00 to 14.01 with a step of 0.01
        for dif_pH in (dif_pH / 100 for dif_pH in range(1401)):
            if dif_pH >= 14.00:
                break
      #calc absolute charge of the molecule at the current pH    
        charge = abs(self._charge_(dif_pH))
      #If current charge is less than the current minimum charge, update the minimum charge
        if charge < MaxCharge:
            MaxCharge = charge
